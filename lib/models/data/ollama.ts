  ...
  try {
    ...
    return data.models.map((model): ModelConfig => {
      ...
      return {
        ...
        apiSdk: async (userId?: string) => await openproviders(modelName as string, undefined, userId),
      }
    })
  } catch (error) {
...
const staticOllamaModels: ModelConfig[] = [
  {
    ...
    apiSdk: async (userId?: string) => await openproviders("llama3.2:latest" as string, undefined, userId),
  },
  {
    ...
    apiSdk: async (userId?: string) => await openproviders("qwen2.5-coder:latest" as string, undefined, userId),
  },
]
